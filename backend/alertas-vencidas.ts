...
import { emailWrapper, emailHeaderAlertas, emailColorBar, emailFooterDark } from '@/backend/email-layout'
...
async function enviarEmail(para: string, nombre: string, reporte: Reporte) {
  ...
  const body = `
    ...
    ${emailFooterDark(['alertas@komared.com', 'Este es un correo automático, por favor no respondas a este mensaje.'])}`

  const { error } = await getResend().emails.send({
    from: FROM_EMAIL,
    to: para,
    subject: `⚠️ Alerta sin resolución: ${reporte.municipio} — ${dias} días`,
    html: emailWrapper(emailHeaderAlertas() + emailColorBar(5) + body),
  })
