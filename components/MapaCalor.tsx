...
export default function MapaCalor() {
  ...
  const renderizarDatos = useCallback((reportes: ReporteMapbox[]) => {
    ...
    // Marcadores (visibles al hacer zoom)
  ...
  const actualizarDatos = useCallback(async () => {
    const { data, error } = await supabase.from('mapa_reportes_publico').select('*')
  ...
  useEffect(() => {
    ...
    // Cargar leaflet.heat dinámicamente (requiere window)
    import('leaflet.heat').then(() => actualizarDatos())

    // Realtime
    const canal = supabase
      .channel('reportes-mapa')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'reportes' }, actualizarDatos)
      .subscribe()
